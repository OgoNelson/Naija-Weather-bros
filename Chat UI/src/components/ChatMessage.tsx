...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  // Custom markdown components for better weather display
  const components = {
    h1: ({children}: any) => (
    ...
    code: ({inline, children}: any) => (
      inline ? (
        <code className="bg-gray-100 px-1 py-0.5 rounded text-xs text-gray-700">{children}</code>
      ) : (
        <code className="block bg-gray-100 p-2 rounded text-xs text-gray-700 whitespace-pre-wrap">{children}</code>
      )
    ),
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`max-w-sm sm:max-w-md lg:max-w-lg ${isUser ? 'order-2' : 'order-1'}`}>
        {/* Sender name for AI messages */}
        {!isUser && (
          <div className="text-xs font-medium text-gray-500 mb-1">{message.senderName}</div>
        )}
